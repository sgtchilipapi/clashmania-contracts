...
library CraftingRecipes {
    ...
    function getMainMaterial(uint256 item_type) internal pure returns (uint256 material, uint256 amount){
        ...
        if(item_type == 1){material = 0; amount = 9 ether;} //ARMORS: THUMPIRON
        if(item_type == 2){material = 0; amount = 6 ether;} //HELMS: THUMPIRON
        if(item_type == 3){material = 0; amount = 3 ether;} //ACCESSORIES: CLINKGLASS
